...
import { convertSearchParamsStr, getLang, toFormData, toObject } from '../../helper';
      ...
      <form
        ...
        <SearchInput
          inputProps={{
            placeholder: search,
            value: values.search,
            onChange: (e) => setValues((p) => ({ ...p, search: e.target.value })),
            name: 'search',
          }}
          clearValue={() => {
            setValues((p) => ({
        ...
        <CstmInput
          ...
          onChange={(e) => setValues((p) => ({ ...p, magnitude: e.target.value }))}
          name="magnitude"
...
const loader = async ({ params: { lang, page = 1 }, request }) => {
  ...
  try {
    ...
    const res = await instance.get(`current-earthquake?lng=${lang}&page=${page}`);
    return {
      data: res.data.data,
